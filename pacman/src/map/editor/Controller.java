...
import src.map.utilities.MapLoader;
...
public class Controller implements ActionListener, GUIInformation {
	...
	/**
	 * The model of the map src.map.editor.
	 */
	public static final String propertiesPath = "properties/test2.properties";
	private Grid model;
	...
	public Controller(int mode, String filename) {
		fileLoader = new MapLoader();
		if (mode == 0){ // open a specific  map
			ArrayList<String> boards = new ArrayList<>();
			String board = fileLoader.loadMap(filename);
			if (board.equals("Failed game checking")){ // failed game checking
				init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
			} else if (board.equals("Failed level checking")){ // failed level checking
				init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
				...
				new Game(gameCallback, properties, boards);
				init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
				loadOneFile(filename);
			}
		} else { // open a folder
			ArrayList<String> boards = fileLoader.loadMaps(filename);
			if (boards.isEmpty()){ // failed game checking
				init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
			} else {
				if (boards.get(boards.size() - 2).equals("Failed level checking")){ // failed level checking
					init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
					loadOneFile(boards.get(boards.size() - 1));
				} else { // play the games in the folder
					final Properties properties = PropertiesLoader.loadPropertiesFile(propertiesPath);
					GameCallback gameCallback = new GameCallback();
					new Game(gameCallback, properties, boards);
					init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
